...
func (r *scriptResolver) ScriptSources(ctx context.Context, obj *models.Script, limit *int, offset *int, filter *ScriptSourceFilterInput, orderBy *string, order ScriptSourceFieldName, orderDirection OrderDirection) ([]*models.ScriptSource, error) {
	if filter == nil {
		filter = &ScriptSourceFilterInput{}
	}
	if filter.And == nil {
		filter.And = []*ScriptSourceFilterInput{}
	}
	filter.And = append(filter.And, &ScriptSourceFilterInput{
		ScriptID: &UUIDFilterInput{Eq: &obj.ID},
	})
	return r.Query().ScriptSources(ctx, limit, offset, filter, orderBy, order, orderDirection)
}
